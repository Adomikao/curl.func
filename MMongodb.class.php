...
class MMongodb {
     ...
     * 返回值：表的记录数
     */
    function count($table_name, $condition=array(), $options=array())
    {
        $dbname = $this->curr_db_name;   
        $options['maxTimeMS'] = 60000;     
        return $this->mongo->$dbname->$table_name->count($condition, $options);
    }
     ...
     * $table_name:表名
     * $condition:更新条件
     * $newdata:新的数据记录  $inc $set $unset $push $pushAll $addToSet $pop $pull $pullAll
     ...
     *
     */
    function updateSet($table_name, $condition, $newdata, $options=array())
    {
        return $this->update($table_name, $condition, array('$set'=>$newdata), $options);
    }
     ...
     * 失败：false
     */
    function delete($table_name, $condition, $options=array())
    {
        $dbname = $this->curr_db_name;
        $options['safe'] = 1;
        try {
     ...
     * 失败：false
     */
    function find($table_name, $query_condition, $result_condition=array(), $fields=array(), $iscursor=0)
    {
        $dbname = $this->curr_db_name;
    ...
    {
        $dbname = $this->curr_db_name;
        return $this->mongo->$dbname->listCollections();
    }
}
